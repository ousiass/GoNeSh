...
**GoNeSh独自ショートカット:** 2つの方式

1. **`?`キー** → ショートカット一覧を表示 → キーを押して実行
2. **`Alt + キー`** → 直接実行
...
| タブ切り替え（次） | `]` | `Alt + ]` | 右へ |
| タブ切り替え（前） | `[` | `Alt + [` | 左へ |
| タブを閉じる | `w` | `Alt + w` | close **w**indow |
| AIパネル表示/非表示 | `a` | `Alt + a` | **a**i |
| プリセット選択 | `p` | `Alt + p` | **p**reset |
| ファイルブラウザ | `f` | `Alt + f` | **f**ile |
| Quick Transfer | `s` | `Alt + s` | **s**end |
| API Client | `r` | `Alt + r` | **r**equest |
| Claude Code送信 | `c` | `Alt + c` | **c**laude |
| 外部AIツール選択 | `x` | `Alt + x` | e**x**ternal |
| Git Auto Commit | `g` | `Alt + g` | **g**it |
| 履歴検索 | - | `Ctrl + R` | (標準) |
...
- `Alt + p` でフローティングメニューからプリセット選択

### 3-4-4. Native AI Integration

- ターミナル内のテキストを選択して `Alt + a` でAIパネルへコンテキスト転送。
...
```bash
# ショートカットで即座に送信
Alt + c  # → 選択テキスト or 現在のコンテキストをClaude Codeに送信
...
```bash
# ショートカットで実行
Alt + g  # → git diffを解析してコミットメッセージ生成
